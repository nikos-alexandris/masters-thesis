...
\ThunkLang{} intermediate representation of the PureCake compiler. We start with
an analysis of \ThunkLang{}'s semantics and the proof methods currently used for
...
\section{The \ThunkLang{} Intermediate Representation}

Each one of PureCake's intermediate representations has its own syntax and
semantics. \ThunkLang{} is the first intermediate language and its structure
resembles that of the source language closely with just two differences:
conversion of the calling convention from call-by-name to call-by-value and
introduction of the \lit{force} and \lit{delay} expressions. We describe its
syntax and semantics in depth to help with the understanding of the logical
...
The set $exp$ describes the language's expressions. \lit{var} is used to denote
variables by name. \lit{Prim} and \lit{Monad} apply either a pure or a monadic
operator to a list of expressions. Application and abstraction are denoted as
$e_1 \cdot e_2$ and $\lambda x.\ e$. \lit{let} and \lit{letrec} denote name
binding, \lit{if-then-else} the usual conditional operator, and \lit{value}
embeds a value into an expression. The \lit{delay}, \lit{force} and \lit{tick}
operators are \ThunkLang{}'s special operators. \lit{delay} and \lit{force} work
as described in \autoref{chap:intro} and the tick operator will be explained in
the next section.
...
encountered, expression $e$ is substituted into each appearance of the
corresponding formal parameter of \hsinl{f}. Primary consequence of this is that
when evaluating a \ThunkLang{} expression, encountering a variable name is
always an error.

\subsection{Semantics}

\ThunkLang{}'s semantics are defined in a big-step functional style
\cite{kahn1987natural}. Big-step semantics are defined by derivation rules for
each construct of the language, and these derivation rules are defined in terms
...
However, when writing code in a formal verification tool such as HOL, every
...
\label{fig:logrel}
\captionof{figure}{\ThunkLang{} Logical Relation.}
\vspace*{\baselineskip}\endgroup

The logical relation consists of a value relation $\mathcal{V}$, a result
relation $\mathcal{R}$, an expression relation $\mathcal{E}$, a variable
relation $\mathcal{X}$, and a substitution relation $\mathcal{C}$. Each relation
is indexed by a clock k and accepts either one or two invariants. The expression
relation takes two invariats, a local invariant $Q_L$ and a global invariant
$Q_G$, while the others take just one invariant $Q$.
...
flexibility is what enables users of the logical relation to decouple their code
from the handling of the clocks by coming up with an appropriate invariant for
...
\cdot e_2)\{Q_L,Q_G\}$. For this to be true, we need to assume $Q_L(0,0)(0,0)$
and $Q_L(k_1,t_1)(k_2,t_2) \Rightarrow Q_L(k_1+1,t_1+1)(k_2+1,t_2+1)$. Finally,
the complete lemma for application compatibility is:
...
\begin{lemma}
  Assume that $Q_L(0,0)(0,0)$ and that $Q_L(c_1,t_1)(c_2,t_2)\Rightarrow\\Q_L(c_1 + 1,t_1 + 1)(c_2 + 1,t_2 + 1)$.
  If
  \begin{itemize}
...
it holds whenever a single application step is made. Similarly, we need to prove
compatibility lemmas for every other language feature.

Finally, the conditions for the invariants to be compatible with the relation
are:

\begin{enumerate}

\item $Q_L(0,0)(0,0)$. This is required for when both expressions time out.

\item $Q_L(1,0)(1,0)$. This is required for the \lit{value} base expression, as
  it occurs a clock step but not a trace step.

\item $Q_L(k_1+1,t_1+1)(k_2+1,t_2+1)$. This is required for the application
  compatibility.

\end{enumerate}

\todo[inline]{TODO: Add more conditions once the compatibility theorems are
proved}
