...
structures. As an example, consider the memory representation of evaluating the
list \hsinl{xs = [1,2,3]} in a strict language. The resulting data structure would
look like a linked list, with a list of pairs each storing a number and a
reference to the next pair in the list.
...
Lastly, the third part in \autoref{chap:cakeml} aims to optimise the
representation of thunks in CakeML. Even with optimisations  which try to
minimise the generation of unnecessary thunks, many thunks will eventually make
it to the runtime. Currently, CakeML doesn't support thunks, so PureCake
compiles them to 2-valued arrays during the transition to CakeML, where the
first cell is used as a flag to distinguish between evaluated and unevaluated
thunks, and the second as the value of the thunk. This prevents
CakeML to differentiate them from any other array, thus blocking the compiler
from performing any meaningful optimisations on them.

One critical optimisation that we plan on enabling when thunks are added to
CakeML is inlining of evaluated thunks. Concretely, a pass can be implemented
during runtime that checks the values on the heap and, if they are indirections
to evaluated thunks, it can inline the values directly and remove these
indirections. For our earlier list examples, this would turn a fully evaluated
lazy list into a strict one, greatly improving the performance of later
traversals \cite{lazygc}.
