...
procedures to transformations over these automata. Then, the rich mathematical
theory of finite automata can be used to prove that the transformations are
correct, therefore proving that the program must also be correct.
...
like containers that wrap a single unevaluated expression, and evaluate their
expression when it's necessary. For example, consider the following Pure
...
mutable array called a store. Each time a \texttt{delay} is executed, a thunk is
produced and appended to the store along with a flag stating that the thunk has
not yet been evaluated. Then, the first \texttt{force} demanding the thunk's
...
to \hsinl{+} in the \texttt{else} case. Therefore, even though this function is
used in a lazy language, its behaviour will always be strict. We also note that
a call-by-value language will optimise this function even further by noticing
it's tail recursive \cite{tailrec}, turning it into a single loop and thus
avoiding even the stack allocations.
...
in \ThunkLang{} that can help reduce boilerplate code and increase code quality
and maintainability.
