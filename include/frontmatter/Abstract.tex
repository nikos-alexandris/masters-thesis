...
is constantly growing. While much work has been done to ensure compilers output
...
formally verified compilers, that guarantee both the compiler's and the
generated code's correctness. The price paid for developing verified compilers
is that every step in the compilation process must be carefully implemented and
formalised in order for the verification system to accept it, which makes
optimisation pipelines much harder to implement.
...
verify critical missing optimisations, both in PureCake's source-to-source
optimisation pipeline and in the CakeML's runtime system, in order to close the
gap between verified and non-verified compilers for lazy languages in terms of
runtime efficiency.
