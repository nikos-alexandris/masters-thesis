...
This optimisation is crucial for both memory and time efficiency of the program;
firstly, it deallocates a value in memory for free and secondly it makes the
program more cache friendly. One could imagine a program that evaluates a list
of integers and then iterates over continuously using them for some mathematical
operation. Without this optimisation, every integer read is almost sure to miss
the cache, whereas this optimisation enables a more compact runtime
representation. Furthermore, it creates opportunities for more optimisations,
for example smarter garbage collection that can detect the list structure after
the inlining of the values and place them packed together in the heap for even
better cache hit frequency.
