...
Moving downwards on the compilation pipeling, this chapter describes our work
...
program stops running and the garbage collector sweeps through the memory
starting at known used locations called roots, and marks all the used memory.
Any unused memory is then freed and the program can start running again with new
memory free to claim. The actual garbage collector for CakeML employs a more
complicated scheme than that \cite{sandberg2019verified}, but this mental model
is enough for our description here.
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    ...
    \node (box1) [box] {Evaluated};
    \node (box2) [box, right=0cm of box1] {Pointer};
    \node (box3) [box, right=0.85cm of box2] {Value};
    \draw[-stealth] (box2.east) -- (box3.west) node[]{};
  \end{tikzpicture}
  \caption{Evaluated Thunk in Memory}
  \label{fig:thunk}
\end{figure}

An evaluated thunk in memory looks like figure \ref{fig:thunk}. The first word
is the flag that states it's been evaluated and the second word is a pointer to
the actual value. Since we now know that these two cells are a thunk and not
just an array, we can instruct the garbage collector to replace the thunk with
the evaluated value when it encounters it. This optimisation is crucial for both
memory and time efficiency of the program; firstly, it deallocates a value in
memory for free and secondly it makes the program more cache friendly. One could
imagine a program that evaluates a list of integers and then iterates over
continuously using them for some mathematical operation. Without this
optimisation, every integer read is almost sure to miss the cache, whereas this
optimisation enables a more compact runtime representation. Furthermore, it
creates opportunities for more optimisations, for example smarter garbage
collection that can detect the list structure after the inlining of the values
and place them packed together in the heap for even better cache hit frequency.
